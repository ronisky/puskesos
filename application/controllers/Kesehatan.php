...
class Kesehatan extends CI_Controller
{
    ...
    {
        $data['title'] = 'Proposal Kesehatan';
        $data['menu1'] = 'Kesehatan';
        $data['user'] = $this->db->get_where('user', ['email' => $this->session->userdata('email')])->row_array();

        $data['kesehatan'] = $this->db->get_where('proposal', ['nama_organisasi' => $this->session->userdata('nama_organisasi')])->result_array();



        $this->form_validation->set_rules('nama', 'Nama', 'required');
        $this->form_validation->set_rules('nama_proposal', 'Nama Proposal', 'required');
        $this->form_validation->set_rules('nis', 'No Induk Kependudukan', 'required');
        $this->form_validation->set_rules('nisn', 'No Induk Kartu Keluarga', 'required');
    ...
    // UBAH KESEHATAN
    public function ubahkesehatan($id)
    {
        $data['title'] = 'Form Edit kesehatan';
        $data['user'] = $this->db->get_where('user', ['email' => $this->session->userdata('email')])->row_array();

        $data['menu1'] = 'kesehatan';
        $data['kesehatan'] = $this->db->get('proposal')->result_array();

        $data['proposal'] = $this->Model->getkesehatanById($id);
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->load->view('templates/sidebar', $data);
            $this->load->view('templates/topbar', $data);
            $this->load->view('kesehatan/ubahkesehatan', $data);
            $this->load->view('templates/footer');
        } else {
            $this->Model->ubahkesehatan();
            $this->session->set_flashdata('flash', 'Data kesehatan Diubah');
            redirect('kesehatan/index');
        }
    }
    // KELUHAN KESEHATAN
    public function laporankeluhankesehatan()
    {
        $data = array();

        $data['title'] = 'Laporan Kesehatan';
        $data['menu1'] = 'Kesehatan';
        $data['user'] = $this->db->get_where('user', ['email' => $this->session->userdata('email')])->row_array();
        // $data['kesehatan'] = $this->db->get_where('proposal', ['nama_organisasi' => $this->session->userdata('nama_organisasi')])->result_array();

        $data['kesehatan'] = $this->db->query("SELECT * from proposal where konfirmasi = 2 and nama_organisasi = 'Bidang Kesehatan' or konfirmasi = 1 and nama_organisasi = 'Bidang Kesehatan'")->result_array();

        $this->load->view('templates/header', $data);
        $this->load->view('templates/sidebar', $data);
        $this->load->view('templates/topbar', $data);
        $this->load->view('kesehatan/laporankeluhankesehatan', $data);
        $this->load->view('templates/footer');
    }
    // Detail Kesehatan
    public function detailkesehatan($id)
    {
        ...
        $data['proposal'] = $this->Model->getDetailKesehatanById($id);

        $this->load->view('templates/header', $data);
        $this->load->view('templates/sidebar', $data);
        $this->load->view('templates/topbar', $data);
        $this->load->view('kesehatan/detailkesehatan', $data);
        $this->load->view('templates/footer');
    }
    ...
    public function export()
    {
        ...
        $excel->setActiveSheetIndex(0)->setCellValue('A1', "DATA PROPOSAL PENGAJUAN KESEHATAN PUSKESOS KATAPANG"); // Set kolom A1 dengan tulisan "DATA SISWA"
        ...
        $excel->setActiveSheetIndex(0)->setCellValue('I3', "DTKS"); // Set kolom E3 dengan tulisan "ALAMAT"
        ...
        $siswa = $this->ExportModel->ExportKesehatan();
        ...
        foreach ($siswa as $data) { // Lakukan looping pada variabel siswa
            ...
            $excel->setActiveSheetIndex(0)->setCellValue('I' . $numrow, $data->dtks);
    ...
    function cetak($id_proposal)
    {
        ...
        $pdf->Cell(34, 6, $proposal['keperluan'], 0, 1); //end of line

        $pdf->Cell(40, 6, '', 0, 0); // first of line
        $pdf->SetFont('Arial', '', 10);
        $pdf->Cell(52, 6, 'Tujuan                                        :', 0, 0);
        ...
        $pdf->Cell(10, 7, '', 0, 1);
        $pdf->Cell(190, 6, 'Melengkapi Persaratan Pembuatan Surat Keterangan Tidak Mampu (SKTM) KE RUMAH SAKIT', 0, 1, 'C');
