...
class Sosial extends CI_Controller
{
    ...
    {
        $data['title'] = 'Proposal Sosial';
        $data['menu1'] = 'Sosial Dan Ekonomi';
        $data['user'] = $this->db->get_where('user', ['email' => $this->session->userdata('email')])->row_array();

        $data['sosial'] = $this->db->get_where('proposal', ['nama_organisasi' => $this->session->userdata('nama_organisasi')])->result_array();
    ...
    // UBAH SOSIAL
    public function ubahsosial($id)
    {
        $data['title'] = 'Form Edit sosial';
        $data['user'] = $this->db->get_where('user', ['email' => $this->session->userdata('email')])->row_array();

        $data['menu1'] = 'sosial';
        $data['sosial'] = $this->db->get('proposal')->result_array();

        $data['proposal'] = $this->Model->getsosialById($id);
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->load->view('templates/sidebar', $data);
            $this->load->view('templates/topbar', $data);
            $this->load->view('sosial/ubahsosial', $data);
            $this->load->view('templates/footer');
        } else {
            $this->Model->ubahsosial();
            $this->session->set_flashdata('flash', 'Data sosial Diubah');
            redirect('sosial/index');
        }
    }
    // LAPORAN KELUHAN
    public function laporankeluhansosial()
    {
        $data = array();

        $data['title'] = 'Laporan Sosial';
        $data['menu1'] = 'Sosial Dan EKonomi';
        $data['user'] = $this->db->get_where('user', ['email' => $this->session->userdata('email')])->row_array();
        // $data['sosial'] = $this->db->get_where('proposal', ['nama_organisasi' => $this->session->userdata('nama_organisasi')])->result_array();
        $data['sosial'] = $this->db->query("SELECT * from proposal where konfirmasi = 2 and nama_organisasi = 'Bidang Sosial' or konfirmasi = 1 and nama_organisasi = 'Bidang Sosial'")->result_array();

        $this->load->view('templates/header', $data);
        $this->load->view('templates/sidebar', $data);
        $this->load->view('templates/topbar', $data);
        $this->load->view('sosial/laporankeluhansosial', $data);
        $this->load->view('templates/footer');
    }
    // detail

    public function detailsosial($id)
    {
        $data['title'] = 'Detail Sosial';
        $data['menu1'] = 'sosial';
        $data['user'] = $this->db->get_where('user', ['email' => $this->session->userdata('email')])->row_array();

        $data['proposal'] = $this->Model->getDetailSosialById($id);

        $this->load->view('templates/header', $data);
        $this->load->view('templates/sidebar', $data);
        $this->load->view('templates/topbar', $data);
        $this->load->view('sosial/detailsosial', $data);
        $this->load->view('templates/footer');
    }
    ...
    public function export()
    {
        ...
        $excel->setActiveSheetIndex(0)->setCellValue('A1', "DATA PROPOSAL PENGAJUAN SOSIAL & EKONOMI PUSKESOS KATAPANG"); // Set kolom A1 dengan tulisan "DATA SISWA"
        ...
        $siswa = $this->ExportModel->ExportSosial();
